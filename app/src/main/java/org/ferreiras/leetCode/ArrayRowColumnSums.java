 ...
 */
public class ArrayRowColumnSums {

  public static void main(String[] args) {

    int[] rowSum = {3,8}, colSum = {4,7};

    int[][] response = restoreMatrix(rowSum, colSum);
  ...
  public static int[][] restoreMatrix(int[] rowSum, int[] colSum) {
    ...
    while ((currentRow < rows) || (currentColumn < columns)) {
      ...
      if (currentRow >= rows) {
        ...
        response[rows - 1][currentColumn] = colSum[currentColumn];
        currentColumn++;
        continue;

      } else if (currentColumn >= columns){

        response[currentRow][columns - 1] = rowSum[currentRow];
        currentRow++;
        continue;

      }

      int valueAdded = Math.min(rowSum[currentRow], colSum[currentColumn]);
      rowSum[currentRow] -= valueAdded;
      colSum[currentColumn] -= valueAdded;
      response[currentRow][currentColumn] = valueAdded;

      if (rowSum[currentRow] == 0) {
        ++currentRow;
      }

      if (colSum[currentColumn] == 0) {
        ++currentColumn;
      }
