...
0 <= nums[i] < 109
 */
public class ArraySortJumbledNumbers {
  public static void main(String[] args) {
    int[] mapping = {8,9,4,0,2,1,3,5,7,6}, nums = {991,338,38};

    int[] response = sortJumbled(mapping, nums);
  ...
  public static int[] sortJumbled(int[] mapping, int[] nums) {
    ...
    for (int num : nums) {
      if (!map.containsKey(num)) {
        map.put(num, helper(num, mapping));
      }
    }

    Integer[] numsBoxed = Arrays.stream(nums).boxed().toArray(Integer[]::new);
    Arrays.sort(numsBoxed, (a, b) -> Integer.compare(map.get(a), map.get(b)));
  ...
  private static int helper(int num, int[] mapping) {
