...
/*
here are n friends that are playing a game. The friends are sitting in a circle and are numbered from 1 to n in clockwise order.
...
public class WinnerCircularGame {
  ...
  public static int findTheWinner(int n, int k) {
    ...
    while (queue.size() > 1) {
      ...
      int personToBeRemoved = (currentIndex + k - 1) % queue.size();
      queue.remove(personToBeRemoved);
      currentIndex = personToBeRemoved;
    }
