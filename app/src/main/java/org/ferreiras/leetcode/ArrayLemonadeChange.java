...
public class ArrayLemonadeChange {

  public static void main(String[] args) {
    int[] bills = {5,5,5,10,20};
    boolean response = lemonadeChange(bills);
  ...
  public static boolean lemonadeChange(int[] bills) {
    ...
    int CHANGE_OF_FIVE = 5, CHANGE_OF_FIFTEEN = 15, price = 5;
    for (int bill : bills) {
      int change = bill - price;

      if (change == CHANGE_OF_FIVE) {
        if (track.getOrDefault(5, 0) == 0) {
          return false;
        } else {
          track.put(5, track.get(5) - 1);
        }
      } else if (change == CHANGE_OF_FIFTEEN) {
