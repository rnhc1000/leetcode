...
encodedText with the help of a matrix having a fixed number of rows.
...
and so on, and finally the yellow cells. The arrow indicates the order in which the
cells are accessed.

For example, if originalText = "cipher" and rows = 3, then we encode it in the following manner:

The blue arrows depict how originalText is placed in the matrix, and the red arrows denote the
order in which encodedText is formed.
In the above example, encodedText = "ch ie pr".

Given the encoded string encodedText and number of rows, return the original string originalText.
...
public class DecodeSlantedCiphertext {
  public static void main(String[] args) {
    String string = "iveo    eed   l te   olc";
    int k = 4;

    String response = decodeCiphertext(string, k);
  ...
  public static String decodeCiphertext(String encodedText, int rows) {
    ...
    String[] string = encodedText.split("\\s{2,}");
