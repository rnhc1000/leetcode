...
You are given a string array pieces of length n, where pieces[i] describes the type (rook, queen, or bishop)
of the ith piece. In addition, you are given a 2D integer array positions also of length n,
where positions[i] = [ri, ci] indicates that the ith piece is currently at the 1-based coordinate
 ...
 */
public class ArrayChessValidMoves {

  public static void main(String[] args) {
    String[] pieces = {"bishop"};
    int[][] positions = {{4, 3}};

    int moves = countCombinations(pieces, positions);
    System.out.println(moves);
  }


  public static int countCombinations(String[] pieces, int[][] positions) {
    List<List<int[]>> allTargets = IntStream.range(0, pieces.length).mapToObj(i -> new ArrayList<int[]>().reversed()).toList();
  ...
  static int count(int idx, int[][] initPositions, List<List<int[]>> allTargets, LinkedList<int[]> targets) {
  ...
  static boolean valid(int[][] initPositions, List<int[]> targets) {
    List<int[]> positions = Arrays.stream(initPositions).map(int[]::clone).toList();
