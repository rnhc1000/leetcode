 ...
 * meetings[i] = [xi, yi, timei] indicates that person xi and person yi have a meeting
 * at timei. A person may attend multiple meetings at the same time.
 ...
 * 1 <= timei <= 105
 * 1 <= firstPerson <= n - 1
 * Inspired in solution provided by
 * https://leetcode.com/dolly2005/
 */
...
public class FindPeopleWithSecret {
  public static void main(String[] args) {
    ...
    int firstPerson = 1;
    List<Integer> list = findAllPeople(n, meetings, firstPerson);
    for (Integer l : list) {
      System.out.print(" " + l);
    }
  }
  public static List<Integer> findAllPeople(int n, int[][] meetings, int firstPerson) {
    ...
    for (int[] value : meetings) maxMeetTime = Math.max(maxMeetTime, value[2]);
